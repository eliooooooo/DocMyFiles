...
async function sendRequest(projectPath, messages) {
	...
	if (enableTokenizer) {
		// Collect the request in a temporary file
		let tmpMessage = fileSync();
		...
		if (requestSize > MAX_TOKENS ) {
			// Update request size variables
			if (requestSize > TOKENS_PER_MINUTES) longRequest = true;
			bigRequest = true;
	...
	if (answer.toLowerCase() === 'yes' ||  answer === ''){
		...
		if (bigRequest) {
			...
			for (let message of messages) {
				({ listRequest, listMessages, listMessagesSize, totalMessages, ignoredFiles } = await processMessage(message, listRequest, listMessages, listMessagesSize, totalMessages, ignoredFiles));
			};
