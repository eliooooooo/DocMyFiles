...
const projectPath = './project/WriteMyCommits/test/'; // Don't forget to custom the project path
...
async function sendRequest(projectPath, messages) {
	...
	try {
		if (enableTokenizer) {
			...
			messagesJson = messagesJson.replace(/"/g, '\\"').replace(/'/g, "\\'");
			exec(`python3 tokenCounter.py "${messagesJson}"`, (error, stdout, stderr) => {
				if (error) {
